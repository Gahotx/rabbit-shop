...
import { getNowOrderAPI, getPreOrderAPI, submitOrderAPI } from '@/services/order'
...
// 获取订单信息
const orderInfo = ref<GetPreOrderRes>()
const getPreOrderDetail = async () => {
  if (query.skuId && query.count) {
    // 立即购买
    const res = await getNowOrderAPI({
      skuId: query.skuId,
      count: query.count
    })
...
const selectedAddress = computed(() => {
  return (
    ...
    orderInfo.value?.userAddresses.find((item) => item.isDefault === 1)
  )
})
...
onUnload(() => {
  addressStore.handleClearAddress()
})
</script>
...
<template>
  ...
  <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
    ...
    <view class="button" :class="{ disabled: !selectedAddress }" @tap="handleSubmitOrder">
      提交订单
    </view>
  </view>
</template>
