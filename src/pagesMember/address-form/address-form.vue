...
// 修改默认地址
const handleChangeDefault: UniHelper.SwitchOnChange = (e) => {
...
const handleSaveAddress = async () => {
  try {
    ...
    if (query.id) {
      // 修改地址
      await updateAddressAPI(query.id, addressForm.value)
    } else {
      // 新建地址
...
<template>
  ...
  <!-- 提交按钮 -->
  <button class="button" @tap="handleSaveAddress">保存并使用</button>
</template>
