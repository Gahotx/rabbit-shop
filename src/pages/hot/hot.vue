...
const getHotRecommendList = async () => {
  ...
  activeSubType.value = res.result.subTypes[0].id
}

// 加载更多数据
const handleLoadMoreData = async () => {
  const currentSubType = subTypes.value.find((item) => item.id === activeSubType.value)
  if (currentSubType!.goodsItems.page < currentSubType!.goodsItems.pages) {
    ...
    const goods = res.result.subTypes.find((item) => item.id === activeSubType.value)!.goodsItems
    currentSubType!.goodsItems.items.push(...goods.items)
  } else {
  ...
  <view class="viewport">
    ...
    <view class="tabs">
      <text
        ...
        :key="item.id"
        :class="['text', activeSubType === item.id ? 'active' : '']"
        @tap="activeSubType = item.id"
        >{{ item.title }}
      </text>
    </view>
    <!-- 推荐列表 -->
    ...
    >
      ...
      <view class="loading-text"> {{ item.isFinish ? '没有更多数据了~' : '正在加载...' }} </view>
    </scroll-view>
  </view>
</template>
